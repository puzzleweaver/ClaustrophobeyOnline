...
public class Player {
	...
	public void moveTo(int id, int nx, int ny) {
		...
		if(oID != 0) ServerMain.data.terr.set(oID, ServerMain.data.terr.get(oID)-1);
		ServerMain.data.terr.set(PID, ServerMain.data.terr.get(PID)+1);
		if(id != -1) {
	...
	public void delete(int id) {
		if(!ghost)
			ServerMain.data.state[x.get(id)][y.get(id)] = (short) (PID-16384);
		else if(ServerMain.data.state[x.get(id)][y.get(id)] == PID-8192)
			ServerMain.data.state[x.get(id)][y.get(id)] = (short) (PID-16384);
		x.remove(id);
		y.remove(id);
